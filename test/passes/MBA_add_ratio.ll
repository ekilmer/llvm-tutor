; RUN: opt --enable-new-pm=0 -load %shlibdir/MBAAdd%shlibext -legacy-mba-add -mba-ratio=1 -S %s  | FileCheck -check-prefix=MBA %s
; RUN: opt --enable-new-pm=0 -load %shlibdir/MBAAdd%shlibext -legacy-mba-add -mba-ratio=0 -S %s  | FileCheck -check-prefix=NO_MBA %s

; RUN: opt --enable-new-pm=0 -load %shlibdir/RIV%shlibext -legacy-riv -analyze %s  | FileCheck %s
; RUN:  opt -load-pass-plugin %shlibdir/RIV%shlibext -passes="print<riv>" -disable-output %s 2>&1 | FileCheck %s

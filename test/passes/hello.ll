; RUN:  opt --enable-new-pm=0 -load %shlibdir/HelloWorld%shlibext --legacy-hello-world -disable-output 2>&1 %s\
; RUN:   | FileCheck %s
; RUN:  opt -load-pass-plugin  %shlibdir/HelloWorld%shlibext -passes=hello-world -disable-output 2>&1 %s\
; RUN:   | FileCheck %s
